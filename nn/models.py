...
def train(train_img, train_lab, label_range, iter=100):
    ...
    # 全连接层初始化
    fc_0_size = 7 * 7 * 32
    fc_w_0 = create_conv((fc_0_size, label_range))
    fc_b_0 = constant((label_range, 1))
    ...
    for t in range(iter):
        ...
        # 第一层池化
        pool_1 = calc_maxpool(conv_1, maxpool, maxpool_step)
        # print('pool_1.shape = ', pool_1.shape)
        # 第二层卷积
        conv_2 = calc_conv(pool_1, conv3_32_0, conv_b_2, conv_step)
        # print('conv_2.shape = ', conv_2.shape)
        # 第二层池化
        pool_2 = calc_maxpool(conv_2, maxpool, maxpool_step)
        ...
        # softmax层反馈
        cost, fc_delta_1 = bp_softmax(sm, lab_mat)
        ...
        conv_delta_2 = bp_pool(conv_2, pool_delta_2, maxpool, maxpool_step)
        # print('conv_delta_2.shape = ', conv_delta_2.shape)

        # 第二层卷积反馈
        pool_delta_1, cc_grad_2 = bp_conv(pool_1, conv3_32_0, conv_delta_2, conv_step)
        # print('pool_delta_1.shape = ', pool_delta_1.shape)

        # 第一层池化反馈
        conv_delta_1 = bp_pool(conv_1, pool_delta_1, maxpool, maxpool_step)
